...
#include "xlnx_enc_dyn_params_priv.h"
...
#ifdef DEBUG
static int32_t print_dyn_params(XlnxDynParams* dyn_params)
{
    xma_logmsg(XMA_DEBUG_LOG, XLNX_DYN_PARAMS_PARSER,
               "[EnDyLib] is_bitrate_changed = %d\n",
               dyn_params->enc_dyn_param.is_bitrate_changed);
    xma_logmsg(XMA_DEBUG_LOG, XLNX_DYN_PARAMS_PARSER,
               "[EnDyLib] bit_rate = %d\n", dyn_params->enc_dyn_param.bit_rate);
    xma_logmsg(XMA_DEBUG_LOG, XLNX_DYN_PARAMS_PARSER,
               "[EnDyLib] is_bframes_changed = %d\n",
               dyn_params->enc_dyn_param.is_bframes_changed);
    xma_logmsg(XMA_DEBUG_LOG, XLNX_DYN_PARAMS_PARSER,
               "[EnDyLib] num_b_frames = %d\n",
               dyn_params->enc_dyn_param.num_b_frames);
    xma_logmsg(XMA_DEBUG_LOG, XLNX_DYN_PARAMS_PARSER,
               "[EnDyLib] is_min_max_qp_changed = %d\n",
               dyn_params->enc_dyn_param.is_min_max_qp_changed);
    xma_logmsg(XMA_DEBUG_LOG, XLNX_DYN_PARAMS_PARSER, "[EnDyLib] min_qp = %d\n",
               dyn_params->enc_dyn_param.min_qp);
    xma_logmsg(XMA_DEBUG_LOG, XLNX_DYN_PARAMS_PARSER, "[EnDyLib] max_qp = %d\n",
               dyn_params->enc_dyn_param.max_qp);
    return 0;
}
#endif
                                     ...
                                     XlnxDynParamFrames* dyn_param_frames)
{
    ...
    if(key != NULL) {
        ...
        if(s_value != NULL) {
            ...
            } else if(!strcmp(key, D_PARAM_MIN_QP)) {
                if((ret = check_dyn_param_value(i_value, key, ENC_MIN_QP,
                                                ENC_MAX_QP, frame_num)) !=
                   XMA_ERROR) {
                    enc_dyn_param->is_min_max_qp_changed =
                        ENC_RUNTIME_PARAMS_CHANGED;
                    enc_dyn_param->min_qp = i_value;
                }
            } else if(!strcmp(key, D_PARAM_MAX_QP)) {
                if((ret = check_dyn_param_value(i_value, key, ENC_MIN_QP,
                                                ENC_MAX_QP, frame_num)) !=
                   XMA_ERROR) {
                    enc_dyn_param->is_min_max_qp_changed =
                        ENC_RUNTIME_PARAMS_CHANGED;
                    enc_dyn_param->max_qp = i_value;
                }
            } else {
                xma_logmsg(XMA_ERROR_LOG, XLNX_DYN_PARAMS_PARSER,
                           "Invalid key=%s for frame=%d in dynamic params "
                           "config file \n",
                           key, frame_num);
                return XMA_ERROR;
            }
            ...
            xma_logmsg(XMA_ERROR_LOG, XLNX_DYN_PARAMS_PARSER,
                       "Missing value for key=%s in dynamic params config file "
                       "for frame=%d \n",
                       key, frame_num);
            return XMA_ERROR;
        }
...
/**
 * validate_dyn_params: Validates dynamic encoder parameters
 *
 * @param dyn_param_frames: Pointer to XlnxDynParamFrames structure
 * @return XMA_APP_SUCCESS or XMA_APP_ERROR
 */
static int32_t validate_dyn_params(XlnxDynParamFrames* dyn_param_frames)
{
    uint32_t          frame_num = dyn_param_frames->frame_num;
    XlnxEncDynParams* enc_dyn_param =
        &dyn_param_frames->dyn_params.enc_dyn_param;

    uint32_t l_min_qp = enc_dyn_param->min_qp;
    uint32_t l_max_qp = enc_dyn_param->max_qp;
    if(l_min_qp || l_max_qp) {
        if(!(l_min_qp && l_max_qp)) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_DYN_PARAMS_PARSER,
                       "Min/Max QP Duple (%d,%d) is incomplete for frame=%d\n",
                       l_min_qp, l_max_qp, frame_num);
            return XMA_ERROR;
        }
        if(l_min_qp > ENC_MAX_QP || l_max_qp > ENC_MAX_QP) {
            xma_logmsg(
                XMA_ERROR_LOG, XLNX_DYN_PARAMS_PARSER,
                "QP should not be greater than 51 (%d,%d) for frame=%d\n",
                l_min_qp, l_max_qp, frame_num);
            return XMA_ERROR;
        }
        if(l_min_qp > l_max_qp) {
            xma_logmsg(XMA_WARNING_LOG, XLNX_DYN_PARAMS_PARSER,
                       "Min QP (%d) cannot be greater than or equal to Max QP "
                       "(%d) for frame=%d. Interchanging...\n",
                       l_min_qp, l_max_qp, frame_num);
            enc_dyn_param->min_qp = l_max_qp;
            enc_dyn_param->max_qp = l_min_qp;
        }
        if(l_min_qp == l_max_qp) {
            xma_logmsg(
                XMA_WARNING_LOG, XLNX_DYN_PARAMS_PARSER,
                "Min QP (%d) is equal to Max QP (%d) for frame=%d. Range is "
                "too narrow causing behaviour to be unexpected\n",
                l_min_qp, l_max_qp, frame_num);
        }
        if(l_min_qp <= ENC_LOW_QP &&
           l_max_qp < l_min_qp + ENC_MIN_LOW_QP_DELTA) {
            xma_logmsg(
                XMA_WARNING_LOG, XLNX_DYN_PARAMS_PARSER,
                "Min QP (%d) and Max QP (%d) delta is too low for this QP "
                "range for frame %d! Adjusting Max QP to be %d\n",
                l_min_qp, l_max_qp, frame_num, l_min_qp + ENC_MIN_LOW_QP_DELTA);
            enc_dyn_param->max_qp = l_min_qp + ENC_MIN_LOW_QP_DELTA;
        }
    }
    return XMA_SUCCESS;
}
                                         ...
                                         XlnxDynParamFrames* dyn_param_frames)
{
    ...
    while((dp = fgets(buf, D_PARAM_MAX_LENGTH, fp)) != NULL) {
        ...
        if(validate_dyn_params(&dyn_param_frames[count]) != XMA_SUCCESS) {
            return XMA_ERROR;
        }
        count++;
    }
                                ...
                                XmaFrame* la_input_xframe, uint32_t index)
{
    ...
    if(sd == NULL) {
        xma_logmsg(XMA_ERROR_LOG, XLNX_DYN_PARAMS_PARSER,
                   "Failed allocating side data in encoder input frame\n");
        return XMA_ERROR;
    }
