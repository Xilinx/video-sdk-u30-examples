                                               ...
                                               XmaParameter* enc_options)
{
    ...
    /* For H.264 encoder multi-core, to disable pipeline we need to enable low
       latency flag in soft kernel */
    const char* AvcLowLat = "DISABLE";
    if(enc_props->avc_lowlat) {
        AvcLowLat = "ENABLE";
    }
                                               ...
                                               XmaParameter* custom_xma_params)
{
    ...
    custom_xma_params[param_cnt].name   = "disable_pipeline";
    custom_xma_params[param_cnt].type   = XMA_UINT32;
    custom_xma_params[param_cnt].length = sizeof(enc_props->disable_pipeline);
    custom_xma_params[param_cnt].value  = &(enc_props->disable_pipeline);
    param_cnt++;
                                           ...
                                           XmaParameter* custom_params)
{
    ...
    custom_params[param_cnt].name = (char*)XLNX_LA_EXT_PARAMS[EParamDynamicGop];
    custom_params[param_cnt].user_type = EParamDynamicGop;
    custom_params[param_cnt].type      = XMA_UINT32;
    custom_params[param_cnt].length    = sizeof(int);
    custom_params[param_cnt].value     = &la_props->dynamic_gop;
    param_cnt++;
    return;
}
