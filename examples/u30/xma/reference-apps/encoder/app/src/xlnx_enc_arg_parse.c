...
static struct option enc_options[] = {
    ...
    {FLAG_DYNAMIC_GOP, required_argument, 0, DYNAMIC_GOP_ARG},
    ...
    {FLAG_DISABLE_PIPELINE, required_argument, 0, DISABLE_PIPELINE_ARG},
    {FLAG_AVC_LOW_LAT, required_argument, 0, AVC_LOWLAT_ARG},
...
char* xlnx_enc_get_help()
{
    ...
    return " XMA Encoder App Usage:\n\t"
           ...
           "INT_MAX\n"
           "\t-dynamic-gop <0/1>         Disable (default) / enable dynamic "
           "gop.\n"
           "\t                           Automatically change B-frame\n"
           "\t                           structure based on motion vectors.\n"
           "\t                           Requires Lookahead_depth of at least "
           "5.\n"
           ...
           "\t                           Serializes encoding (does not affect "
           "lookahead).\n"
           "\t-avc-lowlat <0/1>          Enable AVC low latency flag for \n"
           "\t                           H264 to run on multiple cores incase "
           " of pipeline disabled.\n"
...
static int32_t xlnx_enc_validate_arguments(XlnxEncoderArguments* arguments)
{
    ...
    if(enc_props->dynamic_gop) {
        ...
        if(enc_props->lookahead_depth == 0) {
            xma_logmsg(XMA_WARNING_LOG, XLNX_ENC_APP_MODULE,
    ...
    if(enc_props->disable_pipeline) {
        ...
        if(enc_props->num_bframes == ENC_UNSET_NUM_B_FRAMES) {
            ...
            xma_logmsg(XMA_ERROR_LOG, XLNX_ENC_APP_MODULE,
                       ...
                       "Change encoder parameters to encode only I and P "
                       "frames. Ie -bf 0\n");
            return XMA_APP_ERROR;
        }
    }
                                   ...
                                   XlnxEncoderArguments* arguments)
{
    ...
    while(1) {
        ...
        switch(flag) {
            ...
            case DISABLE_PIPELINE_ARG:
                ret = xlnx_utils_set_int_arg(&enc_props->disable_pipeline,
                                             optarg, FLAG_DISABLE_PIPELINE);
                break;
...
static void xlnx_enc_set_arg_defaults(XlnxEncoderArguments* arguments)
{
    ...
    arguments->expert_options.dynamic_params_check = 0;
    ...
    /* num_b_frames will default to DEFAULT_NUM_B_FRAMES later, depending on
     * dynamic gop and disable_pipeline values.*/
