                                       ...
                                       XlnxEncoderAppCtx*    ctx)
{
    ...
    if(arguments->expert_options.dynamic_params_check) {
        EncDynParams* dyn_params = &ctx->enc_ctx.enc_dyn_params;
        dyn_params->dynamic_param_handle = xlnx_enc_get_dyn_params(
                                           ...
                                           XmaFrame*          la_input_xframe)
{
    ...
    EncDynParams*     enc_dyn_params = &enc_ctx->enc_dyn_params;
    uint32_t          enc_frame_cnt;
    ...
    if((enc_dyn_params->dynamic_params_count > 0) &&
       (enc_dyn_params->dynamic_params_index <
        ...
        uint32_t dyn_frame_num = xlnx_enc_get_dyn_param_frame_num(
            enc_dyn_params->dynamic_param_handle,
            enc_dyn_params->dynamic_params_index);

        if(dyn_frame_num == (enc_frame_cnt)) {
            uint32_t num_b_frames = xlnx_enc_get_runtime_b_frames(
                enc_dyn_params->dynamic_param_handle,
                enc_dyn_params->dynamic_params_index);
            ...
            if(num_b_frames > (uint32_t)enc_ctx->enc_props->num_bframes) {
                xma_logmsg(
                    XMA_ERROR_LOG, XLNX_ENC_APP_MODULE,
