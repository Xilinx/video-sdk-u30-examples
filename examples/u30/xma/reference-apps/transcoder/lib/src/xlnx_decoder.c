...
int32_t xlnx_dec_parse_frame(XlnxDecoderCtx* dec_ctx)
{
    ...
    if(dec_ctx->dec_props.codec_type == DECODER_ID_H264) {
        ret = xlnx_dec_parse_h264_au(dec_ctx->in_file, &dec_ctx->in_buffer,
                                     in_frame_data, 1, &dec_ctx->in_offset);
    } else if(dec_ctx->dec_props.codec_type == DECODER_ID_HEVC) {
        ret = xlnx_dec_parse_hevc_au(dec_ctx->in_file, &dec_ctx->in_buffer,
                                     in_frame_data, 1, &dec_ctx->in_offset);
    } else {
        ret = XMA_APP_ERROR;
    }
    if(ret != XMA_APP_SUCCESS) {
        xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                   "Failed to find first access unit in input file!\nMake "
...
int32_t xlnx_dec_read_frame(XlnxDecoderCtx* dec_ctx)
{
    ...
    XlnxDecFrameData* in_frame_data = &dec_ctx->frame_data;
    if(dec_ctx->dec_props.codec_type == DECODER_ID_H264) {
        if((ret = xlnx_dec_parse_h264_au(
                dec_ctx->in_file, &dec_ctx->in_buffer, in_frame_data, 0,
                &dec_ctx->in_offset)) <= XMA_APP_ERROR) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "Failed to find first unit in h264 input video file!\n");
            return ret;
        }
    } else if(dec_ctx->dec_props.codec_type == DECODER_ID_HEVC) {
        if((ret = xlnx_dec_parse_hevc_au(
                dec_ctx->in_file, &dec_ctx->in_buffer, in_frame_data, 0,
                &dec_ctx->in_offset)) <= XMA_APP_ERROR) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
