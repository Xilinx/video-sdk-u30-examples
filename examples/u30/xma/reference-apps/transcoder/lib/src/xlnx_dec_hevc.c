...
int32_t xlnx_dec_parse_hevc_au(int32_t file, XlnxDecBuffer* dec_buffer,
                               XlnxDecFrameData* parsedata, bool is_first_au,
                               int32_t* retoffset)
{
    ...
    while(true) {
        int32_t endoffset = 0;
        int32_t ret       = xlnx_dec_find_next_start_code(
            file, dec_buffer, startoffset, is_first_au, &endoffset);
        if(ret == XMA_APP_ERROR)
            return XMA_APP_ERROR;
