...
static struct option enc_options[] = {
    ...
    {FLAG_ENC_DYNAMIC_GOP, required_argument, 0, ENC_DYNAMIC_GOP_ARG},
    ...
    {FLAG_ENC_DISABLE_PIPELINE, required_argument, 0, ENC_DISABLE_PIPELINE_ARG},
    {FLAG_ENC_AVC_LOWLAT, required_argument, 0, ENC_AVC_LOWLAT_ARG},
                                ...
                                XmaFilterProperties*  xma_la_props)
{
    ...
    la_props->dynamic_gop       = enc_props->dynamic_gop;
...
void xlnx_enc_context_init(XlnxEncoderCtx* enc_ctx)
{
    ...
    enc_props->num_bframes    = ENC_UNSET_NUM_B_FRAMES;
    ...
    enc_ctx->expert_options.dynamic_params_check = 0;
}
                              ...
                              XmaEncoderProperties* xma_enc_props)
{
    ...
    if(enc_props->dynamic_gop) {
        if(enc_props->num_bframes != ENC_UNSET_NUM_B_FRAMES) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "B-Frames set as %d, but dynamic GOP is enabled! "
                       "Dynamic GOP cannot be enabled "
                       "with a set number of B-Frames!\n",
                       enc_props->num_bframes);
            return XMA_APP_ERROR;
        }
        if(enc_props->lookahead_depth == 0) {
            xma_logmsg(XMA_WARNING_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "Dynamic gop enabled, setting lookahead depth to %d\n",
                       DYNAMIC_GOP_MIN_LOOKAHEAD_DEPTH);
            enc_props->lookahead_depth = DYNAMIC_GOP_MIN_LOOKAHEAD_DEPTH;
        } else if(enc_props->lookahead_depth <
                  DYNAMIC_GOP_MIN_LOOKAHEAD_DEPTH) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "Dynamic GOP enabled, but lookahead depth is %d! "
                       "Lookahead depth must be at least %d "
                       "to run dynamic GOP!\n",
                       enc_props->lookahead_depth,
                       DYNAMIC_GOP_MIN_LOOKAHEAD_DEPTH);
            return XMA_APP_ERROR;
        }
        if(enc_props->disable_pipeline) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "Encoder pipeline is disabled, but dynamic GOP is "
                       "enabled! Encoder pipelining "
                       "cannot be disabled with dynamic GOP!\n");
            return XMA_APP_ERROR;
        }
        if((enc_props->profile == ENC_H264_BASELINE) ||
           (enc_props->profile == ENC_H264_HIGH_10_INTRA) ||
           (enc_props->profile == ENC_HEVC_MAIN_INTRA) ||
           (enc_props->profile == ENC_HEVC_MAIN_10_INTRA)) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "Encoder profile is I and/or P only, enabling dynamic "
                       "GOP results in incorrect delta QPs \n");
            return XMA_APP_ERROR;
        }
    }

    if(enc_props->disable_pipeline) {
        if((enc_props->width * enc_props->height == ENC_SUPPORTED_MAX_PIXELS) &&
           (enc_props->fps > 30)) {
            xma_logmsg(XMA_WARNING_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "Performance may not be run at real time beyond 4k30 "
                       "with encoder pipeline disabled");
        }
        if(enc_props->num_bframes == ENC_UNSET_NUM_B_FRAMES) {
            xma_logmsg(XMA_WARNING_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "Pipeline disabled, setting B-Frames to 0\n");
            enc_props->num_bframes = 0;
        } else if(enc_props->num_bframes != 0) {
            xma_logmsg(
                XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                "Pipeline cannot be disabled when encoding b-frames! "
                "Change encoder parameters to encode only I and P frames.\n");
            return XMA_APP_ERROR;
        }
    }
    /* For dynamic gop, we let b-frames to use default b-frames. This caps the
    number of b-frames dynamic gop can choose. */
    enc_props->num_bframes = enc_props->num_bframes == ENC_UNSET_NUM_B_FRAMES ?
                                 ENC_DEFAULT_NUM_B_FRAMES :
                                 enc_props->num_bframes;

    if(enc_props->codec_id == ENCODER_ID_H264) {
        if(!enc_props->disable_pipeline && enc_props->avc_lowlat) {
            xma_logmsg(
                XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                "AVC low latency flag is not needed when pipeline is enabled");
            return XMA_APP_ERROR;
        } else if(enc_props->disable_pipeline && !enc_props->avc_lowlat) {
            if(enc_props->num_cores > 1) {
                xma_logmsg(
                    XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                    "More than one core given for with pipeline disabled! Set "
                    "AVC low latency to allow for multiple cores with pipeline"
                    "disabled!");
                return XMA_APP_ERROR;
            }
            if((enc_props->width > 1920) || (enc_props->height > 1920)) {
                xma_logmsg(
                    XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                    "Input is %d x %d, but H264 pipeline is disabled. Set AVC"
                    "low latency to disable pipelining for higher resolutions",
                    enc_props->width, enc_props->height);
                return XMA_APP_ERROR;
            }
        }
    }
    ...
    while(!channel_end) {
        ...
        switch(flag) {
            ...
            case ENC_NUM_BFRAMES_ARG:
                ret = xlnx_utils_set_int_arg(&enc_props->num_bframes, optarg,
                                             FLAG_ENC_NUM_BFRAMES);
                break;

            case ENC_DYNAMIC_GOP_ARG:
                ret = xlnx_utils_set_uint_arg(&enc_props->dynamic_gop, optarg,
                                              FLAG_ENC_DYNAMIC_GOP);
                break;
            ...
            case ENC_DISABLE_PIPELINE_ARG:
                ret = xlnx_utils_set_int_arg(&enc_props->disable_pipeline,
                                             optarg, FLAG_ENC_DISABLE_PIPELINE);
                break;

            case ENC_AVC_LOWLAT_ARG:
                ret = xlnx_utils_set_int_arg(&enc_props->avc_lowlat, optarg,
                                             FLAG_ENC_AVC_LOWLAT);
                break;
                         ...
                         XmaFilterProperties*  xma_la_props)
{
    ...
    if(enc_ctx->expert_options.dynamic_params_check) {
        EncDynParams* dyn_params = &enc_ctx->enc_dyn_params;
        dyn_params->dynamic_param_handle =
            xlnx_enc_get_dyn_params(enc_ctx->expert_options.dynamic_params_file,
                                    &dyn_params->dynamic_params_count);
        if(dyn_params->dynamic_param_handle == NULL) {
            xma_logmsg(XMA_ERROR_LOG, XLNX_TRANSCODER_APP_MODULE,
                       "Failed to create dynamic params handle\n");
            return XMA_APP_ERROR;
        }
    }
    return XMA_APP_SUCCESS;
                        ...
                        XmaFilterProperties*  xma_la_props)
{
    ...
    if(enc_ctx->enc_props.enc_options) {
